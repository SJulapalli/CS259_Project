...
llama_context::llama_context(const llama_model & model, llama_context_params params) :
    ...
    if (model.arch == LLM_ARCH_HMT && model.hmt_initial_memory != nullptr &&
        ...
        this->hmt_ctx = ggml_init(params_hmt);

        this->hmt_memory_buffer = ggml_new_tensor_1d(this->hmt_ctx, GGML_TYPE_F32, n_embd);
        ggml_set_name(this->hmt_memory_buffer, "hmt_runtime_memory");

        this->hmt_backend_buffer =
            ggml_backend_buft_alloc_buffer(ggml_backend_cpu_buffer_type(), ggml_nbytes(this->hmt_memory_buffer));

        this->hmt_memory_buffer->buffer = this->hmt_backend_buffer;
...
llama_context::~llama_context() {
    ...
    if (hmt_backend_buffer) {
        ggml_backend_buffer_free(hmt_backend_buffer);
    }
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    if (!balloc->init(batch_inp, vocab, memory.get(), n_embd, cparams.kv_unified ? LLAMA_MAX_SEQ : cparams.n_seq_max,
                      output_all)) {
        LLAMA_LOG_ERROR("%s: failed to initialize batch\n", __func__);
        return -1;
    }
...
std::map<ggml_backend_buffer_type_t, llama_memory_breakdown_data> llama_context::memory_breakdown() const {
    ...
    for (const auto & backend_ptr : backends) {
        ...
        ret[ggml_backend_sched_get_buffer_type(sched.get(), backend)].compute +=
