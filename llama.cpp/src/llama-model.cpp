            ...
            case LLM_ARCH_HMT:
                {
                    ...
                    // HMT Tensors
                    hmt_cross_q = create_tensor(tn(LLM_TENSOR_HMT_CROSS_Q, "weight"), { n_embd, n_embd * 2 }, 0);
                    hmt_cross_k = create_tensor(tn(LLM_TENSOR_HMT_CROSS_K, "weight"), { n_embd, n_embd * 2 }, 0);

                    hmt_summary_prompt =
                        create_tensor(tn(LLM_TENSOR_HMT_SUMMARY_PROMPT, "weight"), { n_embd, 1 }, TENSOR_NOT_REQUIRED);
                    hmt_initial_memory =
                        create_tensor(tn(LLM_TENSOR_HMT_INITIAL_MEMORY, "weight"), { n_embd, 1 }, TENSOR_NOT_REQUIRED);

                    hmt_mem_map_lin = create_tensor(tn(LLM_TENSOR_HMT_MEM_MAP_LIN, "weight"), { n_embd, n_embd },
                                                    TENSOR_NOT_REQUIRED);
                    hmt_mem_map_inv = create_tensor(tn(LLM_TENSOR_HMT_MEM_MAP_INV, "weight"), { n_embd, n_embd },
                                                    TENSOR_NOT_REQUIRED);
            ...
            case LLM_ARCH_JINA_BERT_V2:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        layer.ffn_up = create_tensor(tn(LLM_TENSOR_FFN_UP, "weight", i),
                                                     { n_embd, layer.ffn_gate ? n_ff : n_ff * 2 }, 0);
...
ggml_cgraph * llama_model::build_graph(const llm_graph_params & params) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_LLAMA4:
            ...
            break;
        case LLM_ARCH_HMT:  //HMT Graph Built Switch Case
            {
                llm = std::make_unique<llm_build_hmt>(*this, params);
            }
            break;
        case LLM_ARCH_DECI:
            {
