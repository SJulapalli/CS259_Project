...
@ModelBase.register("HMTModel")
class HMTModel(LlamaModel):
    model_arch = gguf.MODEL_ARCH.HMT
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        match name:
            ...
            case "cross_attn.wk.weight":
                return [("hmt.cross_attn_k.weight", data_torch)]
            case "mem":
                return [("hmt.summary_prompt.weight", data_torch.squeeze())]
            case "memory_cell.mem_map.inv_linear.weight":
                return [("hmt.mem_map.inv.weight", data_torch)]
            case "memory_cell.mem_map.linear.weight":
                return [("hmt.mem_map.linear.weight", data_torch)]
            case "memory_cell.memory":
                return [("hmt.initial_memory.weight", data_torch.squeeze())]

        if "lm_head.weight" in name:
            return [
                    ((self.format_tensor_name(gguf.MODEL_TENSOR.OUTPUT), data_torch)),
                    ((self.format_tensor_name(gguf.MODEL_TENSOR.TOKEN_EMBD), data_torch))
                    ]

        nested_prefix = "memory_cell.model.model."

        if name.startswith(nested_prefix):
            stripped_name = name[len(nested_prefix):]

            if stripped_name == "norm.weight":
                return [((self.format_tensor_name(gguf.MODEL_TENSOR.OUTPUT_NORM), data_torch))]

            return super().modify_tensors(data_torch, stripped_name, bid)
