...
def main():
    ...
    if args.rmt_only or args.baseline_only:
        ...
        if args.hmt_stage_1:
            ...
            if args.load_from_ckpt is not None and args.hmt_stage_2:
                ...
                if os.path.isfile(args.load_from_ckpt):
                    logger.info(f"Loading state dict from file: {args.load_from_ckpt}")
                    state_dict = torch.load(args.load_from_ckpt, map_location='cpu')
                else:
                    logger.info(f"Loading state dict from DeepSpeed checkpoint: {args.load_from_ckpt}")
                    state_dict = get_fp32_state_dict_from_zero_checkpoint(args.load_from_ckpt, tag="hmt_checkpoints")
    ...
    model.memory_cell.model = model.memory_cell.model.merge_and_unload()

    safetensors.torch.save_model(model, "/Users/Suhas/HMT-pytorch/llama3.2-1b-pg19-final-hmt.safetensors")
